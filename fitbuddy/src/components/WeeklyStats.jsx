  ...
  return (
    <div style={styles.barCol}>
      ...
      <span style={{
        ...
        color: isToday ? 'var(--color-accent-primary)' : 'var(--color-text-faded)',
      }}>
        {day.label}
      </span>
    </div>
...
function WeeklyStats({ streak, weeklyData, habitStats }) {
  return (
    <div style={styles.card}>
      ...
      <div className="chart-overflow-wrapper">
      ...
      {habitStats.map(h => {
        ...
        const fillColor = pct >= 80 ? 'var(--color-accent-primary)' : pct >= 50 ? 'var(--color-accent-secondary)' : 'var(--color-accent-tertiary)';
        return (
...
const styles = {
  ...
  metricNum: { fontSize: '26px', fontWeight: '700', color: 'var(--color-accent-secondary)', fontFamily: 'Syne, sans-serif' },
  metricLabel: { fontSize: '12px', color: 'var(--color-text-faded)' },
  divider: { width: '1px', height: '40px', background: 'var(--color-border)' },
  chartTitle: {
    fontSize: '11px', fontWeight: '500', color: 'var(--color-text-faded)',
    textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '12px',
  },
  chart: { display: 'flex', gap: '6px', alignItems: 'flex-end', marginBottom: '16px', minWidth: '320px' },
  ...
  habitName: { fontSize: '13px', fontWeight: '500', color: 'var(--color-text-main)' },
  habitPct: { fontSize: '12px', color: 'var(--color-text-faded)' },
  miniTrack: {
